...
### Ranking variables using penalized IV. Using ncore=2 because more than 2 is no allowed by CRAN in examples  
...
### Create a vector of variables in the training data
...
To see the labels use the `show_values` parameter:

```{r, echo=TRUE, results='asis', fig.width=5, fig.height=5}
PlotWOE(IV, "N_OPEN_REV_ACTS", show_values=TRUE)
```

We can also do a multiplot to compare WOE patterns. Here we plot the first nine variables on one page. Note that we can plot as many variables as we want; MultiPlotWOE will simply spread the plots over multiple pages.
...
```{r, echo=TRUE, results='asis'}
knitr::kable(head(selected_members))
```
...
riv package (not to be confused with the riv packake in CRAN), ([link](http://www.r-bloggers.com/r-credit-scoring-woe-information-value-in-woe-package/]))
